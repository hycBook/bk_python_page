...
def run(domain: str):
    ...
    print(domain)
    pat = r'id=(?P<zj>.*?)\&'
    post_data = {'start': 0, 'nd': 6, 'domain': domain, 'type': 0, 'style': 109, }
    up_base_url = 'https://www.cutercounter.com/getcode.php'
    data = urllib.parse.urlencode(post_data)
    data = data.encode('ascii')
    inner_head = {'user-agent': ua, }
    add = urllib.request.Request(url=up_base_url, headers=inner_head, method='POST')
    response = urllib.request.urlopen(url=add, timeout=timeout, data=data)
    res = response.read().decode('utf-8')
    res = list(re.finditer(pat, res))
    print("**********************************")
    return res[0].groupdict()['zj']


def get_js(js_path: str, base_url: str):
    heads = """
/*
...
"""
    ...
    with open(js_path, 'r', encoding='utf-8') as f:
        for line in f:
            if line.startswith('// ========') or tag:
                tag = True
                tail.append(line)
            else:
                res = re.search(exi_pat, line)
                if res:  # and res.groupdict()['name'].strip():
                    coll[res.groupdict()['name']] = res.groupdict()['mid']
    if "" not in coll:
        mid = run(base_url)
        coll[''] = mid

    return heads, coll, tail


def gen(html_path: str, name: str, mid: str):
    return f'// {html_path}\nid_map.set("{name}", "{mid}");\n'


def fn(base_fix):
    base_path = r'H:\github\tmp_fol\chapters'
    base_path = r'./chapters'
    # base_fix = r'https://dl.hycbook.com'
    prefix = f'{base_fix}/chapters/'
    file_names = ['']
    for file in os.listdir(base_path):
        f_path = os.path.join(base_path, file)
        if os.path.isfile(f_path) and file.endswith('.md'):
            file_names.append(f"{file[:-2]}html")

    js_path = os.path.join(base_path, 'res', 'patch', 'gitbook-plugin-pageview-count', 'plugin.js')
    heads, coll, tail = get_js(js_path, base_url=base_fix)
    gens = []
    ...
    with open(js_path, 'w', encoding='utf-8') as f:
        for ct in chain(heads, gens, tail):
            f.write(ct)
    print("处理结束")
